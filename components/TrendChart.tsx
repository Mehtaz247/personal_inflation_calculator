  ...
  return (
    <div className="rounded-xl border border-ink-200 bg-white p-5 shadow-sm">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-ink-900">Personal vs official inflation, monthly</h3>
        <div className="flex items-center gap-3 text-xs text-ink-600">
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 bg-ink-900" /> Personal
          </span>
          <span className="flex items-center gap-1.5">
            <svg width="16" height="2"><line x1="0" y1="1" x2="16" y2="1" stroke="#71717a" strokeDasharray="3 2" strokeWidth="1.5" /></svg>
            Official
          </span>
        </div>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
        {yTicks.map((t, i) => (
          <g key={i}>
            <line
              ...
              strokeWidth="1"
            />
            <text x={PAD_L - 6} y={yScale(t)} textAnchor="end" dominantBaseline="middle" className="fill-ink-500 text-[10px]">
              {(t * 100).toFixed(1)}%
            </text>
          </g>
        ))}
        {data.map((d, i) =>
          i % xLabelEvery === 0 || i === data.length - 1 ? (
            <text key={d.month} x={xScale(i)} y={H - 12} textAnchor="middle" className="fill-ink-500 text-[10px]">
              {shortMonth(d.month)}
            </text>
          ) : null,
        )}
        <path d={path("official")} fill="none" stroke="#71717a" strokeWidth="1.5" strokeDasharray="4 3" />
        <path d={path("personal")} fill="none" stroke="#18181b" strokeWidth="2" />
      </svg>
    </div>
  );
}
